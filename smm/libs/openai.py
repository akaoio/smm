...
class OpenAI:
    ...
    def request(self, method="POST", url=None, endpoint=None, json=None, data={}, headers={}, **args):
        url = url or self.base_url + endpoint
        headers = self.headers(headers)
        if utils.find(args, "files"):
            del headers["Content-Type"] # If upload files, the content type is generated by requests
        if json is not None or headers.get("Content-Type") == "application/json":
...
def generate_content(**args):
    ...
    if generate_image:
        ...
        if response.status_code == 200:
            ...
            file = requests.get(url=data.get("url"))
            # Convert file content to PNG using PIL and io
            content = to_png_content(file.content)
    ...
    if generate_image_variation:
        ...
        content = to_png_content(file_content)
        files = {'image': content}
